...
def select_12():
    ...
    subquery = (select(func.max(Grade.grade_date)).join(Student).filter(and_(
        Grade.subjects_id == 1, Student.group_id == 1
    ))).scalar_subquery()

    result = session.query(Student.id, Student.fullname, Grade.grade, Grade.grade_date) \
        .select_from(Grade) \
        .join(Student) \
        .filter(and_(Grade.subjects_id == 1, Student.group_id == 1, Grade.grade_date == subquery)).all()
...
def select_11():
    ...
    SELECT
        ...
        ROUND(AVG(g.grade), 2) AS average_grade,
        t.fullname AS teacher_full_name
    FROM students s
    JOIN grades g ON s.id = g.student_id
    JOIN subjects subj ON g.subject_id = subj.id
    JOIN teachers t ON subj.teacher_id = t.id
    ...
    result = session.query(Student.fullname, func.round(func.avg(Grade.grade), 2).label("average_grade"), \
                           Teacher.fullname).select_from(Student).join(Grade).join(Subject).join(Teacher).filter(
        Student.id == 3).filter(Teacher.id == 2).group_by(Student.fullname).group_by(Teacher.fullname).all()
    return result
